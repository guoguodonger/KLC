...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("KLC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("KarenCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non KLC unit input in lnReqAmount, we generate the URI with BKC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::KLC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
